...
public class AvaliarEstrutura {
    ...
    public void validaEstrutura(String conteudo) {
        ...
        if(pilhaTags.estaVazia()){
            mensagemValidade += "Arquivo HTML é válido!";
        } else {
            mensagemValidade += "Arquivo HTML INVÁLIDO!";
        }

        mensagemFinal +=
                "\nFrequência das tags \n" +
                this.verificaFrequenciaTags(this.tagsUtilizadas) +
                "\n" + mensagemValidade;
    ...
    private String verificaFrequenciaTags(Lista<String> lista){
        String texto = "";
        for (int i = 0; i < lista.obterComprimento(); i++) {
            String frequenciaTagAtual = lista.verificaFrequencia(tagsUtilizadas.getNo(i))+"";
            String tagAtual = lista.getNo(i).getInfo();

            texto += "Tag '" + tagAtual + "' utilizada "
                    + frequenciaTagAtual + " vezes \n";
        }
        return texto;
    }
